 ...
 * @return {[type]} [description]
 */
baseTpl.prototype.pcToMobile = function(Router, to) {
	var nowPage = to.matched && to.matched.length && to.matched[0];
	if(!nowPage) return false;

	var eventIsPhone = Vue.appCommonH.isWeixin().isPhone || window.innerWidth <= appConfig.mobileMaxWidth,
...
baseTpl.prototype.changSize = function(to) {
	...
	clientWidth = clientWidth <= appConfig.mobileMaxWidth ? clientWidth : appConfig.mobileMaxWidth;
