 ...
 * 防抖方法
 */
appCommonH.debounce = function(key, callback, wait) {
 ...
 * 获取当前链接地址
 */
appCommonH.getCurrUrl = function () {
...
appCommonH.getUrlParam = function (url) {
  var match = url.indexOf('?') !== -1 ? url.split('?')[1] : '';
  match = match.indexOf("#") !== -1 ? url.split('#')[0] : '';
  if (!match) return {};
