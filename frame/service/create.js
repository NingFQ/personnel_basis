...
module.exports = {
	...
	otherConfig() {
		...
		this.config.modules.forEach((mName) => {
			...
			this.pathConfig.forEach((onePath) => {
				if(fs.existsSync(path.resolve(onePath, mName))) {
					if(fs.existsSync(path.resolve(onePath, mName+'/config/main.config.js'))) {
						let mainConfig = this.requireFile(onePath, mName+'/config/main.config.js');
						
						if(mainConfig.pages) routerConfig = {...routerConfig, ...mainConfig.pages};
						if(mainConfig.apis) apiConfig = {...apiConfig, ...mainConfig.apis};							
					}
					
					if(fs.existsSync(path.resolve(onePath, mName+'/store/index.js'))) hasStore = true;
					if(fs.existsSync(path.resolve(onePath, mName+'/config/pcmobile.config.js'))) {
						let nowPcmobileConfig = this.requireFile(onePath, mName+'/config/pcmobile.config.js');

						pcmobileConfig = {...pcmobileConfig, ...this.changePcmobileToObject(nowPcmobileConfig)};
					}
					modulesDirs.push(path.resolve(onePath, mName));					
				}			
			});
			...
			if(hasStore) allStoreConfig.push({name: mName, path: '../merge/'+mName+'/store/index.js'});
		});
	...
	setDir() {
		...
		this.staticDir = path.resolve(__dirname, '../../public/static');
	},
